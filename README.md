...
<p align="center">
  <img src="https://img.shields.io/badge/Firmware%20beta-3.1.6-orange">
  <img src="https://img.shields.io/badge/Web%20beta-3.1.12-orange">
...
- [Actualización por Bluetooth (OTA)](#actualización-por-bluetooth-ota)
...
Videos sobre **armado, configuración, actualización, pruebas de vuelo, funcionamiento y novedades** del proyecto **F5LControl**.

👉 **[Visitar F5LControl en YouTube](https://www.youtube.com/@F5LControl)**
---
## Características
...
- Actualización de firmware por Bluetooth (OTA) — desde la versión 3.1.0.
...
**VERSION FW:** `3.1.6` 🧪 (beta)  
**VERSION WEB:** `3.1.12` 🧪 (beta)
...
## Actualización por Bluetooth (OTA)

<p align="center">
  <img src="IMAGENES/LOGO_OTA_BT.png" alt="OTA por Bluetooth" width="130">
</p>

> ✅ Disponible **a partir de la versión 3.1.0** del firmware.

**OTA** (del inglés *Over-The-Air*, "por el aire") significa que el equipo puede **actualizar su firmware por Bluetooth, sin cables ni programas**. Se hace desde la misma **Web Control**, en el celular o la PC.

### Dos formas de actualizar

- **Automática (la más fácil):** con el botón **"Buscar actualizaciones"**, la web mira si hay una versión más nueva publicada y, si la hay, **descarga e instala la última versión disponible** sola, con barra de progreso.
- **Por archivo (avanzada):** subís vos un archivo `firmware.bin` y la web lo instala. Sirve para **volver a una versión anterior** (rollback) o para probar una versión puntual que no sea la última.

En las dos, durante la actualización la pantalla del equipo muestra **UPDATE** con el porcentaje. Si algo se corta o falla, **no pasa nada**: el equipo sigue con la versión anterior (la nueva recién queda activa cuando se verificó completa).

### Seguridad

- El equipo **no está "escuchando" actualizaciones** todo el tiempo: la OTA se inicia **solo cuando vos se lo pedís** desde la web.
- La imagen se valida (tamaño + verificación de integridad) **antes** de activarse.

### ⚠️ Importante: el salto a la 3.1.X se hace una vez por cable

Como la versión **3.1.X cambió la organización interna de la memoria** (para poder tener dos firmwares y hacer OTA seguro), **la primera vez que pasás de una versión 3.0.X a la 3.1.X hay que flashear por USB**.

En ese flasheo por cable es **obligatorio grabar los 3 archivos** siguiendo el **manual de flasheo**:

- `bootloader.bin`
- `partitions.bin`
- `firmware.bin`

> ⚠️ No alcanza con grabar solo el `firmware.bin`: sin la tabla de particiones nueva (`partitions.bin`) el equipo no arranca ni queda habilitado el OTA.

A partir de ese flasheo, **todas las actualizaciones siguientes ya son por Bluetooth**.

> En resumen: **3.0.X → 3.1.X = por cable, los 3 `.bin` según el manual (una sola vez)**. **3.1.X → 3.1.Y en adelante = por Bluetooth (OTA)**.
...
| Listo para volar | Altura en metros (número grande) + tilde de OK |
...
La altura a la que la pantalla se enciende sola al aterrizar es configurable desde la web (hasta 10 m; 10 m por defecto).
...
> ⚠️ **Bajo ningún motivo reinicies la placa con el botón de RESET (RST).**

**Versión CON pantalla (LCD):** para reiniciar, usá **siempre el botón BOOT** manteniéndolo apretado (el tiempo configurado, 2 s por defecto) **con la pantalla encendida** (es decir, en el piso). Eso hace un **reinicio controlado**: primero **cierra y graba el vuelo**, pone el **motor en mínimo (failsafe)** y **recién ahí reinicia**. Seguro y sin perder datos. Se puede habilitar/deshabilitar y ajustar el tiempo desde la web (sección "Pantalla").

**Versión SIN pantalla:** el reinicio controlado con el botón **NO funciona** (necesita la pantalla). En ese caso, **el único medio de reset es desconectar la batería** — **no** usar el botón RST.

**Por qué evitar el RST:** reinicia el micro **de golpe** → si estabas en un vuelo podés **perder los últimos datos** que no se alcanzaron a grabar, y el ESC puede recibir un transitorio en el arranque.
